...
static constexpr const u32 PSF1_FONT_WIDTH    = 8;
static constexpr const u32 PSF1_HEADER_SIZE   = sizeof(PSF1Font);
static constexpr const u32 PSF1_MODE512       = 0x01;

static constexpr const u64 ANSI_COLOR_BLACK   = 0x6d30005b1b;
static constexpr const u64 ANSI_COLOR_RED     = 0x6d31005b1b;
static constexpr const u64 ANSI_COLOR_GREEN   = 0x6d32005b1b;
static constexpr const u64 ANSI_COLOR_YELLOW  = 0x6d33005b1b;
static constexpr const u64 ANSI_COLOR_BLUE    = 0x6d34005b1b;
static constexpr const u64 ANSI_COLOR_MAGENTA = 0x6d35005b1b;
static constexpr const u64 ANSI_COLOR_CYAN    = 0x6d36005b1b;
static constexpr const u64 ANSI_COLOR_WHITE   = 0x6d37005b1b;

static constexpr const u64 ANSI_RESET         = 0x6d305b1b;

PSF1Font*                  font               = (PSF1Font*)zap_vga09_psf;
// u32                 backgroundColor  = 0x383c3c;

bool                       Terminal::Initialize(Framebuffer& framebuffer)
{
    this->framebuffer = framebuffer;
    if (this->framebuffer.address) initialized = true;
...
void Terminal::PutChar(u64 c)
{
    ...
    const u32 glyphCount       = font->mode == PSF1_MODE512 ? 512 : 256;
    u32       charactersPerRow = framebuffer.width / (PSF1_FONT_WIDTH + 2);
    u32       charactersPerCol = framebuffer.height / (font->charSize + 2);
    ...
    switch (c)
    {
        ...
        case '\t':
        {
            static constexpr const u8 tabSize = 4;
            u8                        toSkip  = x % tabSize;
            x += tabSize - toSkip;
        }
        break;
        ...
        default:
        {
            u32 bytesPerLine = (PSF1_FONT_WIDTH + 7) / 8;

            u8* glyph        = (u8*)font + PSF1_HEADER_SIZE
                      + (c > 0 && c < glyphCount ? c : 0) * font->charSize;

            u32 ypos = y * font->charSize;
            u32 xpos = x * PSF1_FONT_WIDTH + 1;
            for (u32 _y = ypos; _y < font->charSize + ypos; _y++)
            {
                u32 mask = 1 << (PSF1_FONT_WIDTH - 1);
                for (u32 _x = xpos; _x < PSF1_FONT_WIDTH + xpos; _x++)
                {
                    PutPixel(*((u32*)glyph) & mask ? foregroundColor
                                                   : backgroundColor,
                             _x, _y);
                    mask >>= 1;
                }
...
void Terminal::PrintString(const char* string, usize length)
{
    while (length > 0)
    {
        if (*string == 0x1b)
        {
            usize offset = UpdateColors(string);
            //            if (length >= offset)
            //            {
            length -= offset;
            string += offset;
...
void Terminal::ScrollDown(u8 lines)
{
    usize bytesToCopy
        = framebuffer.pitch * (framebuffer.height - lines * font->charSize);
    void* src = (void*)(reinterpret_cast<uintptr_t>(framebuffer.address)
                        + framebuffer.pitch * font->charSize * lines);
    memcpy((void*)framebuffer.address, (void*)src, bytesToCopy);

    if (y >= lines) y -= lines;
    else y = 0;
}

void Terminal::PutPixel(u32 pixel, u32 x, u32 y)
{
    u32* pixelOffset
        = (u32*)(reinterpret_cast<uintptr_t>(framebuffer.address)
                 + y * framebuffer.pitch + (x * (framebuffer.bpp / 8)));
    *pixelOffset = pixel;
}
usize Terminal::UpdateColors(const char* character)
{
    u64 colorCode = *reinterpret_cast<u64*>(const_cast<char*>(character));
